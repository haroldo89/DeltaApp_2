...
LayoutHelper.authorizeConfigurationModuleServices = function () {
    ...
    var configurationModuleServicesResult = $("#hdnUserPermission").val();
    if (!CommonUtils.isNullOrEmpty(configurationModuleServicesResult)) {
        ...
        var configurationServices = $("#ulModulesApp li ul li");
        ...
        $.each(configurationServices, function (i, configurationService) {
            var serviceMenuId = $(configurationService).attr("data-name");
            var serviceAvailable = Enumerable.from(configurationModuleServices.UserTypeModulePermission).firstOrDefault(function (m) { return m.MODULES_ALIAS.trim() == serviceMenuId });
            if (!serviceAvailable) {
                $("#ulModulesApp li ul li[data-name=" + serviceMenuId + "]").remove();
            }
        });
    }
    else {
